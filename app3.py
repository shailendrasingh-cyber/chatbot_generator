...
def setup_nltk_data():
    ...
    try:
        ...
        # Configure NLTK to use our directory first
        ...
        if not punkt_tab_path.exists():
            ...
            punkt_tab_path.parent.mkdir(parents=True, exist_ok=True)
            
            try:
                urllib.request.urlretrieve(punkt_tab_url, zip_path)
                with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                    zip_ref.extractall(punkt_tab_path.parent)  # Extract to tokenizers directory
                os.remove(zip_path)
            except Exception as e:
                st.error(f"Failed to download punkt_tab: {str(e)}")
                return False
        ...
        try:
            ...
            st.error("NLTK resources not found after installation attempt")
            return False
